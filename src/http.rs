    ...
    impl LoadDriver for HttpOptions {
        fn load_driver(&self) -> Result<(), Box<::std::error::Error>> {
            ...
            let resp = match self.http_verb.as_ref() {
                "POST" => http_helpers::post_request,
                "GET" => http_helpers::get_request,
                _ => http_helpers::get_request, // defaults
            };
            ...
            while now.elapsed() <= dur {
                ...
                for i in 0..rps {
                    ...
                    if i == (rps - 1) && now.elapsed() <= dur {
                        ...
                        let sleep_time = 1000 as u32 - elapsed_time
                            .duration_since(execution_time)
                            .subsec_millis();
